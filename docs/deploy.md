...
### ทำไมตัวจริงไม่ได้อยู่ใน vercel.json

แพลน Hobby ของ Vercel ตั้ง cron ได้ต่ำสุด **วันละครั้ง** และคลาดได้ถึง ±59 นาที
ถ้า `vercel.json` ตั้งเป็น `* * * * *` Vercel จะ **ปฏิเสธตั้งแต่ตอน import project**
ไม่ใช่แค่เตือน — deploy ไม่ผ่านเลย

`vercel.json` จึงตั้งไว้วันละครั้งเพื่อให้ deploy ผ่าน และทำหน้าที่เป็น**ตาข่ายรองรับ**
เท่านั้น **ตัวจริงต้องเป็น scheduler ข้างนอกที่ยิงทุกนาที** เช่น cron-job.org
ซึ่งฟรีและตั้งความถี่ระดับนาทีได้ — ถ้าไม่ตั้ง ข้อความยืนยันการจองจะไปถึงลูกค้า
ช้าได้ถึงหนึ่งวัน

ถ้าย้ายขึ้น Vercel Pro เมื่อไร เปลี่ยน `vercel.json` กลับเป็น `* * * * *`
แล้วปิด scheduler ข้างนอกได้

worker ดึงงานด้วย `FOR UPDATE SKIP LOCKED` จึงรันซ้อนกันได้โดยไม่ส่งซ้ำ
ตัวข้างนอกกับตัวของ Vercel ยิงชนกันก็ไม่เป็นไร
ถ้า cron หยุดไป ข้อความจะค้างในคิว ไม่หาย — พอกลับมาจะทยอยส่งต่อ
**ข้อควรระวัง:** worker เช็คแค่ **สถานะ** ของ booking ตอนส่ง ไม่ได้เช็คเวลา
คิวที่ถูกยกเลิกหรือไม่มาตามนัดจะไม่ถูกเตือน แต่คิวที่ยัง `confirmed`
หรือ `completed` ไปแล้วยังได้รับข้อความอยู่ — ถ้า cron หยุดไปหลายชั่วโมง
ลูกค้าจะได้ "เตือนนัด" ของนัดที่ผ่านไปแล้ว นี่คือเหตุผลที่ความถี่ของ cron
สำคัญกว่าที่เห็น ไม่ใช่แค่เรื่องความช้า
