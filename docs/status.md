...
> อ่าน [§ 14 กันยายน 2026](#-14-กันยายน-2026--ขึ้น-production--ผลงานร้าน) ก่อน
...
**ผลทดสอบล่าสุด (14 ก.ย. 2026):** 145 unit ผ่าน · typecheck / lint / build ผ่าน

⚠️ **integration (12 ไฟล์) กับ e2e ยังไม่ได้รันรอบนี้** — เครื่องที่ทำงานไม่มี
Postgres ในเครื่อง และ `DATABASE_URL_TEST` ยังชี้ไป localhost ตัวเลข
"107 integration + 22/22 e2e" ด้านล่างเป็นของ 13 ก.ย. **ต้องรันใหม่ก่อนเชื่อ**
โดยเฉพาะ 4 ไฟล์ loyalty ที่ถูกแก้ให้ mock `LOYALTY_ENABLED` (ดูด้านล่าง)
...
- **cron ยังไม่ได้ตั้ง scheduler ข้างนอก** — `vercel.json` ตั้งไว้วันละครั้ง
  (`0 1 * * *`) เพื่อให้ Hobby ยอม deploy เท่านั้น ตัวจริงต้องยิงทุกนาที
  ตาม `docs/deploy.md` ถ้ายังไม่ตั้ง ข้อความจะช้าได้ถึงหนึ่งวัน
