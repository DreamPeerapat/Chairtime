...
/** The owner-link code, as generated by lib/line/owner-link.ts. */
const LINK_CODE = /^CT-[A-Z2-9]{6}$/i;
...
export async function handleEvent(
  ...
  // Checked before every keyword: the shop owner sending the code from the
  // dashboard is claiming the alerts for this phone. The shape (CT-XXXXXX) is
  // distinctive enough that no customer types it by accident, and an unclaimed
  // code falls through to the normal replies rather than announcing itself.
  if (lineUserId && LINK_CODE.test(text.replace(/\s+/g, ''))) {
    const claimed = await claimOwnerLink(tx, ctx.tenantId, text, lineUserId);
    if (claimed) {
      return {
